...
function slugToTag(slug: string): string {
  // Convert slug back to potential tag matches
  return decodeURIComponent(slug).toLowerCase();
}

export default async function TagPage({ params }: PageProps) {
  const { slug } = await params;
  const searchSlug = slugToTag(slug);

  // Find articles with this tag
  const taggedArticles = artikel.filter((a) =>
    a.tags?.some(
      (tag) =>
        tag.toLowerCase().replace(/\s+/g, "-").replace(/\./g, "") === searchSlug ||
        tag.toLowerCase() === searchSlug
    )
  );
      ...
      <main className="max-w-4xl mx-auto px-4 pt-8 pb-12">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-600 dark:text-gray-400 mt-2">
            {taggedArticles.length} {taggedArticles.length === 1 ? "Artikel" : "Artikel"}
          </p>
        </div>
